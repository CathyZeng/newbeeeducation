...
public class NewBeeMallOrderServiceImpl implements NewBeeMallOrderService {
    ...
    @Override
    public PageResult getInprogressCoursesPage(PageQueryUtil pageUtil) {
        List<NewBeeMallOrder> newBeeMallOrders = newBeeMallOrderMapper.findNewBeeMallOrderList(pageUtil);
        //获取我的课程信息
        List<Long> orderIds = new ArrayList<>();
        for(NewBeeMallOrder item:newBeeMallOrders){
        ...
        if (!CollectionUtils.isEmpty(orders)) {
            ...
            if (StringUtils.isEmpty(errorOrderNos)) {
                ...
                if (errorOrderNos.length() > 0 && errorOrderNos.length() < 100) {
                    return errorOrderNos + "订单的状态不是支付成功无法开启课程";
                } else {
                    return "你选择了太多状态不是支付成功的订单，无法执行开启课程";
                }
            }
        }
        //未查询到数据 返回错误提示
