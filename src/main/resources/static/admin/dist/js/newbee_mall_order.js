    ...
    $("#jqGrid").jqGrid({
        ...
        colModel: [
            ...
            {label: '订单号', name: 'orderNo', index: 'orderNo', width: 120},
             {label: '课程名称', name: 'totalPrice', index: 'totalPrice', width: 120},
            {label: '课程总价', name: 'totalPrice', index: 'totalPrice', width: 60},
...
//        swal(orderNos + "订单的状态不是支付成功无法执行课程开始完成操作", {
//            icon: "error",
//        });
//        return;
//    }
...
function orderCheckOut() {
    ...
    if (orderNos.length > 0 & orderNos.length < 100) {
        swal(orderNos + "订单的状态不是支付成功无法执行课程开始操作", {
            icon: "error",
        });
        return;
    }
    if (orderNos.length >= 100) {
        swal("你选择了太多状态未支付成功，无法执行课程开始操作", {
            icon: "error",
        });
        return;
    }
